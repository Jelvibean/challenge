 ...
 * - isisLoading: a boolean that is true when the fetch is in progress and false otherwise
...
export const useCachingFetch: UseCachingFetch = (url) => {
  ...
  useEffect(() => {
    if (cache.has(url)) return; // means it found it in the cache already stored.

    let canceled = false;

    const fetchData = async () => {
      try {
        setisLoading(true); // setting the isLoading when initializing
...
/**
 * 2. Implement a preisLoading caching fetch function. The function should fetch the data.
 *
